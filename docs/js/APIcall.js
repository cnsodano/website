...
async function fetchCrossrefMember(prefix) {
    const url = `https://api.crossref.org/v1/prefixes/${prefix}`;
    try {
...
async function fetchCrossrefMemberIDByQuery(query) {
    const url = `https://api.crossref.org/v1/members?query=${query}`;
    try {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
...
async function fetchCrossrefJournals(query) {
    // debugger;
    const url = `https://api.crossref.org/v1/journals?query=${query}&rows=30`;
    try {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
...
async function displayJournalQueryResults(query, data, iBreak = 30, tableSelector, showAll = false) {
    ...
    let apiHeader = `<a href = "https://api.crossref.org/v1/journals?query=${queryAPI}" > API request:</a><code class="inline-code">"https://api.crossref.org/v1/journals?query=${queryAPI}"</code>`
    let result;
    if (noClone === 1) {
...
$("#failMemberFind").css("visibility", "hidden") // Initial
...
async function getJournalsFromPublisher(memberID, maxNumJournals) {
    let url = `https://api.crossref.org/v1/members/${memberID}/works?facet=container-title:${maxNumJournals}&rows=0`;
    try {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
